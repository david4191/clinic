	...
	while($data = mysqli_fetch_assoc($getPateintData)){
						...
						<?php
								if(isset($_POST['send'])){
									...
									$invoiced = mysqli_query($conn,"update appoint set apt_stats='$apt' where pat_id='$patid' ");
										echo'
							<script>
    ...
    else
    {
        $('[name="amount45"]').val("");
						...
						echo $subtotal; ?><br></strong>
										...
										<input type="hidden" name="apt_stats" value="Paid">
										<input type="hidden"  name="hosno" value="<?php echo $data['ihosno']; ?>">
										<input type="hidden"  name="pat_id" value="<?php echo $data['patid']; ?>">
										<button type="submit" name="send" class="btn btn-warning btn-block">Confirm Payment</button> <br>
